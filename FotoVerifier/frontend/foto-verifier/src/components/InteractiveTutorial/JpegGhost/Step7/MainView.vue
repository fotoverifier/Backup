...
export default {
  ...
  setup(props, context) {
    ...
    let src = ref(origin.value + "/demo-jpeg-ghost/upload");
    let url = ref("");
    ...
    let showHint = function () {
      ...
      wn.postMessage("step-7-show-hint", window.location.origin);
    };
    ...
    let iframeEvent = function (event) {
      if (typeof event.data !== "undefined" && event.data !== null) {
        if (event.data == "step-7-hide-hint") {
          context.emit("hideHint");
        } else if (event.data == "step-7-incorrect-image") {
          context.emit("incorrectImage");
        } else if (event.data == "step-7-correct-image") {
