...
export default {
  ...
  setup(props, context) {
    ...
    let src = ref(origin.value + "/demo-jpeg-ghost/image-information");
    let url = ref("");
    ...
    let showHint1 = function () {
      ...
      wn.postMessage("step-8-show-hint-1", window.location.origin);
    };

    let showHint2 = function () {
      let wn = document.getElementById("browser-iframe").contentWindow;
      wn.postMessage("step-8-show-hint-2", window.location.origin);
    };

    let showHint3 = function () {
      let wn = document.getElementById("browser-iframe").contentWindow;
      wn.postMessage("step-8-show-hint-3", window.location.origin);
    };
    ...
    let iframeEvent = function (event) {
      if (typeof event.data !== "undefined" && event.data !== null) {
        if (event.data == "step-8-hide-hint-1") {
          context.emit("hideHint1");
        } else if (event.data == "step-8-move-to-step-2") {
          step.value = 2;
          context.emit("moveToStep2");
        } else if (event.data == "step-8-hide-hint-2") {
          context.emit("hideHint2");
        } else if (event.data == "step-8-move-to-step-3") {
          step.value = 3;
          context.emit("moveToStep3");
        } else if (event.data == "step-8-hide-hint-3") {
