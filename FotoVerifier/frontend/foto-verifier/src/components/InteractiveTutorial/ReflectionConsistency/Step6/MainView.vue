...
export default {
  ...
  setup(props, context) {
    ...
    let src = ref(origin.value + "/demo-reflection-consistency/upload");
    let url = ref("");
    ...
    let showHint = function () {
      ...
      wn.postMessage("step-6-show-hint", window.location.origin);
    };
    ...
    let iframeEvent = function (event) {
      if (typeof event.data !== "undefined" && event.data !== null) {
        if (event.data == "step-6-hide-hint") {
          context.emit("hideHint");
        } else if (event.data == "step-6-incorrect-image") {
          context.emit("incorrectImage");
        } else if (event.data == "step-6-correct-image") {
