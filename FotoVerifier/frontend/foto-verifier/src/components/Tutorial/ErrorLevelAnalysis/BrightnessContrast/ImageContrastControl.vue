  ...
  setup() {
    ...
    let showImage = function () {
      ...
      img.onload = function () {
        ...
        let realContrast = contrast.value * 2.55;
        let factor = (255 + realContrast) / (255.01 - realContrast);
        ...
        for (let y = 0; y < canvas.value.height; y++) {
          for (let x = 0; x < canvas.value.width; x++) {
            ...
            canvasData.data[i + 1] =
              factor * (canvasData.data[i + 1] - 128) + 128;
            canvasData.data[i + 2] =
              factor * (canvasData.data[i + 2] - 128) + 128;
          }
        }
