  ...
  setup() {
    ...
    let iframeEvent = function (event) {
      if (typeof event.data !== "undefined" && event.data !== null) {
        if (event.data == "step-7-show-hint") {
    ...
    let hideHint = function () {
      ...
      window.parent.postMessage("step-7-hide-hint", window.location.origin);
    };

    let incorrectImage = function () {
      window.parent.postMessage(
        "step-7-incorrect-image",
        window.location.origin
      );
    };

    let correctImage = function () {
      window.parent.postMessage("step-7-correct-image", window.location.origin);
    };
    ...
    let handleUploadImageByURL = async function (e) {
      ...
      if (!imageURL) {
        ...
        if (
          imageURL !=
          origin + "/image/interactive-tutorial/jpeg-ghost/sample.jpg"
        ) {
          isUploading.value = false;
          incorrectImage();
        } else {
