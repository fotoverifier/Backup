<template>
  <SubSidebar
    ...
    @checkReflectionConsistency="checkReflectionConsistency"
    @resetImage="resetImage"
    @toggleDisplayReflectionConstraint="toggleDisplayReflectionConstraint"
  />
      ...
      <div class="col-sm-12 p-2">
        ...
        <center>
          <WorkingImage
            ref="workingImage"
            @resetTool="resetTool"
            @updateReflectionConstraints="updateReflectionConstraints"
...
<script>
import SubSidebar from "@/components/ReflectionConsistency/SubSidebar.vue";
import WorkingImage from "../components/ReflectionConsistency/WorkingImage.vue";
import { ref } from "vue";
...
export default {
  ...
  setup() {
    ...
    let checkReflectionConsistency = async function () {
      console.log("checkReflectionConsistency");
      let isConsistent = await workingImage.value.checkReflectionConsistency();
    ...
    let updateReflectionConstraints = function (castShadowConstraints) {
      subSidebar.value.updateReflectionConstraints(castShadowConstraints);
    };

    let toggleDisplayReflectionConstraint = function (id) {
      workingImage.value.toggleDisplayReflectionConstraint(id);
    };
